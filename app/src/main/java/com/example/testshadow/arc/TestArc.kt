...
class TestArc(context: Context, attrs: AttributeSet?, defStyleAttr: Int) :
    ...
    val path = Path()
    val radius = 80f // todo should be calculated
    ...
    private fun drawRightLine(canvas: Canvas) {
        ...
        val dx = 30f
        val shadowColor1 = Color.RED
        val paint2 = Paint().also {
            it.style = Paint.Style.FILL
            it.isAntiAlias = true
            it.setShadowLayer(shadow, dx, dy, shadowColor1)
        }
        path.apply {
            moveTo(viewWidth - radius, 0F)
